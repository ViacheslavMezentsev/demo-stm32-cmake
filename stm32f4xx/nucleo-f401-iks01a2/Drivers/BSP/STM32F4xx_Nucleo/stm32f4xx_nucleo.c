...
static uint32_t IsUsart6MspCbValid = 0;
#endif
__weak HAL_StatusTypeDef MX_USART6_UART_Init(UART_HandleTypeDef* huart);
/**
 * @}
 */
...
int32_t BSP_COM_Init(COM_TypeDef COM)
{
  ...
  else
  {
    ...
    /* Init the UART Msp */
    USART6_MspInit(&hcom_uart[COM]);
#else
    if(IsUsart6MspCbValid == 0U)
    {
...
#endif
    if (MX_USART6_UART_Init(&hcom_uart[COM]))
    {
      ret = BSP_ERROR_PERIPH_FAILURE;
    }
  }
...
int32_t BSP_COM_DeInit(COM_TypeDef COM)
{
  ...
  else
  {
    ...
    #if (USE_HAL_UART_REGISTER_CALLBACKS == 0U)
      USART6_MspDeInit(&hcom_uart[COM]);
...
#if (USE_HAL_UART_REGISTER_CALLBACKS == 1U)
/**
 * @brief Register Default USART6 Bus Msp Callbacks
 * @retval BSP status
 */
int32_t BSP_COM_RegisterDefaultMspCallbacks(COM_TypeDef COM)
{
  ...
  else
  {
    ...
    if(HAL_UART_RegisterCallback(&hcom_uart[COM], HAL_UART_MSPINIT_CB_ID, USART6_MspInit) != HAL_OK)
    {
      ret = BSP_ERROR_PERIPH_FAILURE;
    }
    else if(HAL_UART_RegisterCallback(&hcom_uart[COM], HAL_UART_MSPDEINIT_CB_ID, USART6_MspDeInit) != HAL_OK)
    {
      ret = BSP_ERROR_PERIPH_FAILURE;
    }
    else
    {
      IsUsart6MspCbValid = 1U;
    }
  }
...
/**
 * @brief Register USART6 Bus Msp Callback registering
 * @param Callbacks pointer to USART6 MspInit/MspDeInit callback functions
 * @retval BSP status
 */
...
#endif /* USE_COM_LOG */
/**
 * @brief  Initializes USART6 MSP.
 * @param  huart USART6 handle
 * @retval None
 */
...
static void USART6_MspInit(UART_HandleTypeDef* uartHandle)
{
  ...
  /* USER CODE END USART6_MspInit 0 */
    ...
    */
    ...
    GPIO_InitStruct.Alternate = BUS_USART6_TX_GPIO_AF;
    HAL_GPIO_Init(BUS_USART6_TX_GPIO_PORT, &GPIO_InitStruct);

    GPIO_InitStruct.Pin = BUS_USART6_RX_GPIO_PIN;
    GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
    GPIO_InitStruct.Alternate = BUS_USART6_RX_GPIO_AF;
    HAL_GPIO_Init(BUS_USART6_RX_GPIO_PORT, &GPIO_InitStruct);
...
static void USART6_MspDeInit(UART_HandleTypeDef* uartHandle)
{
  ...
  /* USER CODE END USART6_MspDeInit 0 */
    ...
    */
    ...
    HAL_GPIO_DeInit(BUS_USART6_RX_GPIO_PORT, BUS_USART6_RX_GPIO_PIN);
