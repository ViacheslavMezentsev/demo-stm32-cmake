...
const uint16_t LedPin = GPIO_Pin_3 | GPIO_Pin_4 | GPIO_Pin_5 | GPIO_Pin_6 | GPIO_Pin_7 |
...
static void ConfigLed( void )
{
    ...
    // Источник данных массив DmaBuff.
    Dma.DMA_MemoryBaseAddr = (uint32_t) DmaBuff;

    // Приемник - регистр BSRR порта GPIOB.
    Dma.DMA_PeripheralBaseAddr = ( uint32_t ) & ( GPIOB->BSRR );

    // Периферия является приемником данных.
    Dma.DMA_DIR = DMA_DIR_PeripheralDST;

    // Размер источника данных в "словах".
    Dma.DMA_BufferSize = 4;

    // Не изменять адрес приемника.
    Dma.DMA_PeripheralInc = DMA_PeripheralInc_Disable;

    // Увеличивать адрес памяти.
    ...
    // Не копирование из памяти в память.
    ...
    Tim.TIM_Prescaler = SystemCoreClock / 10000 - 1;

    // Переполнение таймера 400 раз в секунду.
    Tim.TIM_Period = 10000 / 400 - 1;

    TIM_TimeBaseInit( TIM1, &Tim );

    // Генерация события при переполнении таймера.
    TIM_GenerateEvent( TIM1, TIM_EventSource_Update );

    // Разрешаем запуск DMA по событию переполнения.
    TIM_DMACmd( TIM1, TIM_DMA_Update, ENABLE );

    // Разрешаем работу 2 канала модуля DMA1.
    DMA_Cmd( DMA1_Channel5, ENABLE );

    // Разрешаем работу таймера TIM2.
    TIM_Cmd( TIM1, ENABLE );
}
