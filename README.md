...
Все проекты переведены на использование "велосипедного" фреймворка сборки **[stm32-cmake-yml](https://github.com/ViacheslavMezentsev/stm32-cmake-yml)**. Это позволяет отказаться от сложного редактирования `CMakeLists.txt` в пользу простого и декларативного файла конфигурации **`stm32_config.yml`**.

🎥 **[Смотреть краткую видео-инструкцию: Конфигурирование сборки проектов STM32 при помощи скрипта YAML](https://www.youtube.com/watch?v=mdJv4qNZnEY)**
...
Каждая папка проекта является самодостаточной (содержит свои исходники и конфиги), но использует общие настройки VS Code и внешние модули сборки и библиотеки.
...
*Примечание: Убедитесь, что путь к `arm-none-eabi-gcc` добавлен в системную переменную `PATH` (не обязательно, если компиляторов несколько).*
...
Вам больше не нужно править сложные CMake-файлы. Практически вся основная настройка производится в файле **`stm32_config.yml`** в корне папки конкретного проекта.
...
validate_linker_script: true # Проверять соответствие размера RAM в скрипте выбранному мк
```

### Процесс сборки в VS Code

1. Откройте папку репозитория (или конкретного проекта) в VS Code.
2. Выберите используемую сборку компилятора (`arm-none-eabi-gcc`), если CMake Tools спросит.
3. Нажмите **Crtl+Shift+P** и выберите `CMake: Удаление кэша и перенастройка`
4. Нажмите **F7** (или кнопку "Build" в нижней панели).
...
*Обратите внимание: система может сама найти драйверы, сформировать ld-скрипт и проверить корректность параметров памяти.*
...
**Важно про папки в `sources`:** Если вы указываете папку (например, `Core`), внутри неё должен быть свой `CMakeLists.txt`, который подключает содержимое. Если вы используете структуру CubeMX, вам  придется создать эти файлы вручную или перечислить все `.c` файлы в списке `sources` поштучно. Если структура локальной папки `Drivers` соответствует формируемой при помощи CubeMX, то она может подключиться автоматически. Посмотрите как это сделано в примерах.
...
* **Debug (qemu):** Для запуска в эмуляторе (удобно для тестов без железа). Поддерживает выбор машины (Netduino и др.) через `inputs`.
...
Настраивает пути к инструментам (OpenOCD, GDB) в зависимости от ОС (Windows/Linux). Вам может потребоваться подправить пути, если они отличаются.
